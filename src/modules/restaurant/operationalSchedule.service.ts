...
export class OperationalScheduleService {
     ...
     */
    public parseOperationalSchedule(operationalSchedule: string, restaurantId?: number): OperationalSchedule[] {
        ...
        for (let scheduleItem of operationalSchedule.split('/')) {
            ...
            daysBatches.forEach((daysBatch) => {
                if (daysBatch.indexOf('-') !== -1) {
                    ...
                    operationalSchedules.push(this.createOperationalScheduleObject(daysBatch, parsedTime.openAt, parsedTime.closeAt, restaurantId));
                }
            });
        }
     ...
     * @returns {OperationalSchedule[]}
     */
    private parseDaysRange(daysRange: string, operationalTime, restaurantId?: number): OperationalSchedule[] {
        ...
        for (let i = days.indexOf(startDay); i <= days.indexOf(endDay); i++) {
            schedule.push(this.createOperationalScheduleObject(days[i], operationalTime.openAt, operationalTime.closeAt, restaurantId));
        }
        return schedule;
    }
     ...
     * @returns {OperationalSchedule}
     */
    private createOperationalScheduleObject(day: string, openAt: string, closeAt: string, restaurantId?: number): OperationalSchedule {
